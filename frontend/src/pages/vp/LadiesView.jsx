...
export default function LadiesView() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setLadies(ladiesRes.data.members || [])
  ...
  const sendLadiesUpdate = async () => {
    ...
    try {
      ...
      toast.success('Update sent to all ladies!')
      setUpdateTitle(''); setUpdateBody(''); setShowUpdateForm(false)
    } catch (err) { toast.error(err.response?.data?.error || 'Failed to send') }
    finally { setSending(false) }
  }

  const sendHospitalityUpdate = async () => {
    if (!updateTitle.trim() || !updateBody.trim()) return toast.error('Title and message required')
    setSending(true)
    try {
      await api.post('/ministry-content', {
        ministry_name: 'Hospitality Ministry',
        content_type: 'announcement',
        title: updateTitle.trim(),
        body: updateBody.trim(),
      })
      toast.success('Update sent to Hospitality Ministry!')
      setUpdateTitle(''); setUpdateBody(''); setShowUpdateForm(false)
    } catch (err) { toast.error(err.response?.data?.error || 'Failed to send') }
    finally { setSending(false) }
  }

  const currentData = tab === 'ladies' ? ladies : hospitality
    ...
    <div>
      <div className="page-header">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowUpdateForm(!showUpdateForm)}
            className="btn-outline btn-sm"><Send size={13} /> Send Update</button>
      ...
      {showUpdateForm && (
        <div className="card p-5 mb-5 border-l-4 border-teal">
          <h3 className="font-montserrat font-bold text-navy mb-1">Send Update</h3>
          <p className="text-xs text-gray-400 mb-3">
            {tab === 'ladies'
              ? 'This message will appear in announcements for all female members only.'
              : 'This message will appear in the Hospitality Ministry section for all Hospitality members.'}
          </p>
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                onClick={tab === 'ladies' ? sendLadiesUpdate : sendHospitalityUpdate}
                disabled={sending} className="btn-primary">
                {sending ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> : <><Send size={14} /> Send to {tab === 'ladies' ? 'All Ladies' : 'Hospitality Ministry'}</>}
              </button>
