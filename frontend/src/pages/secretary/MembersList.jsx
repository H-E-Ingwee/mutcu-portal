      ...
      <div className="page-header">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link to="/secretary/members/pending" className="btn-outline btn-sm relative">
            Pending Approvals
            {pendingCount > 0 && <span className="absolute -top-1.5 -right-1.5 bg-orange text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{pendingCount}</span>}
          </Link>
          <Link to="/secretary/members/import" className="btn-teal btn-sm"><Upload size={14} />Import CSV</Link>
      ...
      <div className="card">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="text-center py-8"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-orange mx-auto" /></td></tr>
              ) : members.length === 0 ? (
                ...
                return (
                  <tr key={m.id}>
                    ...
                    <td>
                      {m.enrollment_status==='active' ? <span className="badge badge-green">Active</span>
                        : m.enrollment_status==='pending' ? <span className="badge badge-orange">Pending</span>
