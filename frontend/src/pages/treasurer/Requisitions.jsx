...
const UNIT_OPTIONS = ['pieces', 'kg', 'litres', 'metres', 'boxes', 'reams', 'packets', 'sets', 'pairs', 'hours', 'days', 'months', 'trips', 'other']
  ...
  const handlePrint = () => {
    ...
    <style>
      * { margin:0;padding:0;box-sizing:border-box; }
      body { font-family:Arial,sans-serif;color:#1a1a2e;background:#fff;padding:24px; }
      .header { display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:16px;border-bottom:3px solid #04003D; }
      .header-left h1 { font-size:16px;font-weight:800;color:#04003D;margin-bottom:2px; }
      .header-left p { font-size:9px;color:#9CA3AF;letter-spacing:1px;text-transform:uppercase; }
      .header-left .motto { font-size:9px;color:#FF9700;font-style:italic;margin-top:2px; }
      .header-right { text-align:right; }
      .header-right .logo-placeholder { width:60px;height:60px;border:2px solid #04003D;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:800;color:#04003D;text-align:center;line-height:1.2; }
      .doc-title { text-align:center;font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:2px;color:#04003D;margin-bottom:16px;padding:8px;background:#F5F7FA;border-radius:4px; }
      .meta-grid { display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px; }
      .meta-item label { font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#9CA3AF;display:block;margin-bottom:2px; }
      .meta-item span { font-size:11px;font-weight:600;color:#04003D; }
      table { width:100%;border-collapse:collapse;margin-bottom:16px; }
      th { background:#04003D;color:#FF9700;padding:7px 10px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:1px; }
      td { padding:6px 10px;border-bottom:1px solid #F0F0F0;font-size:10px;color:#374151; }
      tr:nth-child(even) td { background:#FAFAFA; }
      .totals { display:flex;justify-content:flex-end;gap:24px;margin-bottom:20px;padding:10px 0;border-top:2px solid #04003D; }
      .total-item { text-align:right; }
      .total-item label { font-size:8px;font-weight:700;text-transform:uppercase;color:#9CA3AF;display:block; }
      .total-item span { font-size:13px;font-weight:800;color:#04003D; }
      ...
      .sig-line { border-top:1px solid #9CA3AF;padding-top:6px; }
      .sig-name { font-size:10px;font-weight:700;color:#04003D; }
      .sig-role { font-size:8px;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.5px;margin-top:1px; }
      .sig-date { font-size:8px;color:#9CA3AF;margin-top:3px; }
      .notes-box { background:#FEF3C7;border:1px solid #F59E0B;border-radius:4px;padding:8px 12px;margin-bottom:12px;font-size:10px; }
        ...
        <div ref={printRef} className="p-6">
          ...
          <div className="header">
            <div className="header-left">
              <h1>MURANG'A UNIVERSITY OF TECHNOLOGY CHRISTIAN UNION</h1>
              <p>MUTCU Digital Management System</p>
              <p className="motto">Inspire Love, Hope & Godliness</p>
            </div>
            <div className="header-right">
              <div className="logo-placeholder">MUTCU<br/>LOGO</div>
            </div>
          </div>
          ...
          <div className="meta-grid">
            ...
            <div className="meta-item"><label>Status</label><span>{statusCfg.label}</span></div>
          ...
          {requisition.review_notes && (
            <div className="notes-box"><strong>Treasurer's Notes:</strong> {requisition.review_notes}</div>
          )}
          ...
          {/* Signatures — space only, no lines overlaying */}
          <div className="signatures">
            ...
            <div className="sig-box">
              ...
              <div className="sig-line">
                ...
                <div className="sig-role">Ministry Chairperson (EC)</div>
            ...
            <div className="sig-box">
              <div className="sig-space"></div>
              <div className="sig-line">
                <div className="sig-name">{requisition.reviewer?.name || '_______________'}</div>
                <div className="sig-role">CU Treasurer</div>
                <div className="sig-date">Date: {requisition.reviewed_at ? new Date(requisition.reviewed_at).toLocaleDateString('en-GB') : '_______________'}</div>
              </div>
            </div>
            <div className="sig-box">
              <div className="sig-space"></div>
          ...
          <div className="footer">MUTCU Digital Management System · portal.mutcu.org · © 2025 Murang'a University of Technology Christian Union</div>
        </div>
      </div>
    </div>
  )
...
export default function Requisitions() {
  const { user, canManageRequisitions } = useAuth()
  ...
  const [newForm, setNewForm] = useState({
    ...
    items: [{ description: '', quantity: 1, unit: 'pieces', unit_cost: 0, total: 0 }],
  })
  const [reviewForm, setReviewForm] = useState({ status: '', total_approved: '', review_notes: '' })
  const [showReview, setShowReview] = useState(false)

  const isTreasurer = TREASURER_ROLES.includes(user?.role)
  const isApprover = APPROVAL_ROLES.includes(user?.role)
  const canEndorse = ENDORSE_ROLES.includes(user?.role)
  ...
  const addItem = () => setNewForm(f => ({ ...f, items: [...f.items, { description: '', quantity: 1, unit: 'pieces', unit_cost: 0, total: 0 }] }))
  ...
  const submitRequisition = async () => {
    if (!newForm.title || newForm.items.some(i => !i.description)) return toast.error('Title and all item descriptions are required')
    setSaving(true)
    try {
      ...
      setShowNew(false)
      setNewForm({ title: '', ministry: user?.primary_ministry || '', purpose: '', spiritual_year: '', ministry_chairperson_id: '', items: [{ description: '', quantity: 1, unit: 'pieces', unit_cost: 0, total: 0 }] })
  ...
  const endorseRequisition = async (id) => {
    setSaving(true)
    try {
      const { data } = await api.put(`/requisitions/${id}/endorse`, {})
      toast.success(data.message)
      fetchRequisitions(); setSelected(null)
    } catch (err) { toast.error(err.response?.data?.error || 'Failed') }
    finally { setSaving(false) }
  }
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      toast.success(data.message)
      fetchRequisitions(); setShowReview(false); setSelected(null)
    } catch (err) { toast.error(err.response?.data?.error || 'Failed') }
    finally { setSaving(false) }
  }
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{canViewAll ? `${requisitions.length} total` : 'My requisitions'}</p>
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <div className="card p-3 space-y-2">
            <div className="relative">
              ...
              <input type="text" className="form-input pl-9 text-sm" placeholder="Search..."
          ...
          <div className="card overflow-hidden flex-1">
            {loading ? (
              ...
              <div className="text-center py-10 text-gray-400 text-sm"><FileText size={32} className="mx-auto mb-2 text-gray-200" />No requisitions found.</div>
            ) : filtered.map(r => {
        ...
        <div className="lg:col-span-3">
          {!selected ? (
            <div className="card h-full flex items-center justify-center">
              <div className="text-center"><FileText size={48} className="text-gray-200 mx-auto mb-3" /><div className="text-gray-400 text-sm">Select a requisition to view details</div></div>
            </div>
          ) : (
            <div className="card overflow-hidden">
              <div className="card-header">
                <div>
                  ...
                  <div className="text-xs text-gray-400">{selected.ministry || 'General'} · KES {parseFloat(selected.total_requested).toLocaleString()}{selected.total_approved ? ` · Approved: KES ${parseFloat(selected.total_approved).toLocaleString()}` : ''}</div>
                </div>
              ...
              <div className="p-5 space-y-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 320px)' }}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-gray-200">
                      ...
                      <th className="text-right py-2 text-gray-400 font-semibold">Unit</th>
                    ...
                    <tbody>
                      {(typeof selected.items === 'string' ? JSON.parse(selected.items) : selected.items || []).map((item, i) => (
                        <tr key={i} className="border-b border-gray-50">
                          ...
                          <td className="py-2 text-right text-gray-500">{item.unit || 'pieces'}</td>
                      ...
                      <tr className="border-t-2 border-navy">
                        <td colSpan={4} className="py-2 font-bold text-navy text-right">Total Requested:</td>
                      ...
                      {selected.total_approved && (
                        <tr>
                          <td colSpan={4} className="py-1 font-bold text-navy text-right">Total Approved:</td>
                ...
                <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                  ...
                  {[
                    ...
                    { label: 'Disbursed by Treasurer', value: selected.status === 'disbursed' ? 'Funds released' : null, date: selected.disbursed_at, done: !!selected.disbursed_at },
                  ].map((step, i) => (
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  {['under_review', 'endorsed'].includes(selected.status) && isApprover && (
                    <button onClick={() => approveRequisition(selected.id)} disabled={saving} className="btn-teal btn-sm">
                      <CheckCircle size={14} />Approve & Sign
                    </button>
                  )}
                  {/* Disburse — CU Treasurer ONLY */}
                  {selected.status === 'approved' && isTreasurer && (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" className="form-input" value={newForm.ministry} onChange={e => setNewForm(f => ({ ...f, ministry: e.target.value }))} />
                </div>
                <div>
                  <label className="form-label">Spiritual Year</label>
                  <input type="text" className="form-input" placeholder="e.g. 2026/2027" value={newForm.spiritual_year} onChange={e => setNewForm(f => ({ ...f, spiritual_year: e.target.value }))} />
                </div>
                <div className="col-span-2">
                  <label className="form-label">Purpose / Justification</label>
                  <textarea className="form-input" rows={2} value={newForm.purpose} onChange={e => setNewForm(f => ({ ...f, purpose: e.target.value }))} />
                </div>
                {canViewAll && members.length > 0 && (
                  <div className="col-span-2">
                    ...
                    <select className="form-select" value={newForm.ministry_chairperson_id} onChange={e => setNewForm(f => ({ ...f, ministry_chairperson_id: e.target.value }))}>
                      ...
                      {members.filter(m => ['ec_admin','super_admin','1st_vp','2nd_vp','music_coordinator','creative_arts_coordinator','tech_media_coordinator','prayer_coordinator','missions_coordinator','bible_study_coordinator','discipleship_coordinator'].includes(m.role)).map(m => (
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-12 gap-1 text-xs text-gray-400 font-semibold px-1">
                    ...
                    <div className="col-span-3">Unit Cost (KES)</div>
                    <div className="col-span-1"></div>
                  </div>
                  {newForm.items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-1 items-center">
                      <input type="text" className="form-input col-span-4 text-sm" placeholder="Description"
                        value={item.description} onChange={e => updateItem(idx, 'description', e.target.value)} />
                      <input type="number" className="form-input col-span-2 text-sm" placeholder="Qty" min="1"
                        value={item.quantity} onChange={e => updateItem(idx, 'quantity', e.target.value)} />
                      <select className="form-select col-span-2 text-sm" value={item.unit} onChange={e => updateItem(idx, 'unit', e.target.value)}>
                      ...
                      {newForm.items.length > 1 ? (
                        <button type="button" onClick={() => removeItem(idx)} className="col-span-1 text-red hover:text-red/70 flex justify-center"><X size={14} /></button>
          ...
          <div className="card p-6 max-w-md w-full">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea className="form-input" rows={3} value={reviewForm.review_notes}
