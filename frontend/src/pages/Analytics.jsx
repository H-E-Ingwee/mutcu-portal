...
export default function Analytics() {
  ...
  const ministryChart = {
    labels: ministryData?.map(m => m.name.replace(' Ministry','')) || [],
    datasets: [{ data: ministryData?.map(m => m.count) || [], backgroundColor: COLORS, borderWidth: 2, borderColor: '#fff' }]
  }
  ...
  const schoolChart = {
    ...
    datasets: [{ label: 'Members', data: schoolData?.map(s => s.count) || [], backgroundColor: '#FF9700', borderRadius: 6 }]
  }
  ...
  return (
    <div>
      <div className="page-header"><div><h1 className="page-title">Analytics Dashboard</h1><p className="page-subtitle">Membership and nomination data insights</p></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="card">
          <div className="card-header"><h2 className="font-montserrat font-bold text-navy text-sm">Ministry Distribution</h2></div>
          <div className="card-body"><Doughnut data={ministryChart} options={doughnutOptions} /></div>
        </div>
        <div className="card">
          <div className="card-header"><h2 className="font-montserrat font-bold text-navy text-sm">Year of Study Distribution</h2></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="card">
          <div className="card-header"><h2 className="font-montserrat font-bold text-navy text-sm">Gender Ratio</h2></div>
          <div className="card-body"><Doughnut data={genderChart} options={{ ...doughnutOptions, plugins: { ...doughnutOptions.plugins, legend: { position: 'bottom', labels: { font: { family: 'Lato', size: 12 } } } } }} /></div>
        </div>
        <div className="card">
          <div className="card-header"><h2 className="font-montserrat font-bold text-navy text-sm">School Distribution</h2></div>
