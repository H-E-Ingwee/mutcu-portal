...
// Render blocks all outbound SMTP ports (587, 465). Uses Brevo REST API instead.
...
async function sendApprovalEmail(user) {
  ...
  return sendEmail({ to: user.email, subject: `MUTCU Membership Approved — Welcome, ${user.name}!`, html })
}
...
async function sendPasswordResetEmail(user, token) {
  ...
  const html = htmlWrap(
    ...
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${resetUrl}" style="display:inline-block;background:#04003D;color:#FF9700;font-weight:700;font-size:14px;padding:14px 36px;border-radius:8px;text-decoration:none;">
  ...
  return sendEmail({ to: user.email, subject: 'MUTCU DMS — Password Reset Link', html })
}
...
async function sendMemberMessageNotification({ senderName, senderEmail, mutcuNumber, subject, body, adminEmails }) {
  ...
  const html = htmlWrap(
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#4A5568;line-height:1.6;">A member has sent a message through the MUTCU portal.</p>
  ...
  return sendEmail({ to: adminEmails.join(', '), subject: `[MUTCU Portal] New Message: ${subject}`, html, replyTo: senderEmail })
}
...
async function sendRejectionEmail(user, reason) {
  ...
  return sendEmail({ to: user.email, subject: 'MUTCU Membership Application — Update', html })
}
...
async function verifyConnection() {
  if (!BREVO_API_KEY) {
    console.warn('[EMAIL] ⚠️  BREVO_API_KEY not set — email sending disabled')
    return false
  }
  try {
    ...
    if (response.ok) {
      ...
      console.log(`[EMAIL] ✅ Brevo API verified — account: ${data.email || 'connected'} | email is ready`)
      return true
    } else {
      console.error('[EMAIL] ❌ Brevo API key invalid — check BREVO_API_KEY env var')
      return false
    }
  } catch (err) {
    console.error('[EMAIL] ❌ Brevo API check failed:', err.message)
    return false
  }
}
