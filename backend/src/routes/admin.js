...
router.put('/roles/:userId', authenticate, requireRole('super_admin'), async (req, res) => {
  try {
    ...
    supabase.from('audit_logs').insert({
      ...
      description: `Role changed for ${data.name} (${data.email}): ${oldRole} → ${role} | Changed by: ${req.user.name}`,
    }).then(() => {}).catch(() => {})

    res.json({ user: data, message: `Role updated to ${role}`, old_role: oldRole })
  } catch (err) { res.status(500).json({ error: err.message }) }
})

// GET /api/admin/role-audit-log — role change audit log
router.get('/role-audit-log', authenticate, requireRole('super_admin', 'ec_admin'), async (req, res) => {
  try {
    const { from, to, page = 1, limit = 50 } = req.query
    let query = supabase.from('audit_logs')
      .select('*, actor:actor_id(id,name,email,role)', { count: 'exact' })
