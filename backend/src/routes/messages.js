...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const { data: admins } = await supabase.from('users')
      .select('email, name').in('role', ['super_admin', 'ec_admin', 'cu_secretary'])
      .eq('is_active', true)
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
            <div style="background:#04003D;padding:1.5rem;text-align:center;">
              <h2 style="color:#FF9700;margin:0;">MUTCU DMS — New Message</h2>
            </div>
            <div style="padding:1.5rem;">
              ...
              <div style="background:#F5F7FA;border-radius:8px;padding:1rem;margin-top:1rem;">
                <p style="color:#333;white-space:pre-wrap;">${body}</p>
              </div>
              <p style="color:#718096;font-size:0.75rem;margin-top:1rem;">Reply directly to ${sender.email}</p>
            </div>
            <div style="background:#F5F7FA;padding:1rem;text-align:center;font-size:0.72rem;color:#718096;">
              Inspire Love, Hope & Godliness · portal.mutcu.org
            </div>
          </div>
        `,
      }).catch(() => {})
