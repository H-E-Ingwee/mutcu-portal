...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    supabase.from('users').select('email, name')
      .in('role', ['super_admin', 'ec_admin', 'cu_secretary', 'vice_secretary', 'cu_treasurer'])
      .eq('is_active', true)
      .then(({ data: admins }) => {
...
router.get('/', authenticate, async (req, res) => {
  try {
    if (!['super_admin','ec_admin','cu_secretary','vice_secretary','cu_treasurer'].includes(req.user.role)) {
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    if (!['super_admin', 'ec_admin', 'cu_secretary', 'vice_secretary'].includes(req.user.role)) {
      return res.status(403).json({ error: 'Access denied' })
    }
